<section id="intro_main">
    <article>
        <h2>아이디어허브는<br>유일하고 유익한 <br class="mo">아이디어를 <br class="pc">연결합니다</h2>
        <p>우수특허권과 투자자를 연결하는<br>IP WAY MAKER</p>
    </article>
</section>
<section id="intro_2">
    <article>
        <div class="left_container">
        <h2>인류의 역사는<br>위대한 발명의 역사입니다.</h2>
        <p>완성된 발명이 ‘특허권'이라는 권리로 머물러 있는게 아니라 <br class="pc">
        상품화되고 재화로 환원될 수 있다면- <br>
        발명가는 Motivation을 얻고, 기업은 글로벌 시장으로의 발판을<br class="pc">
        마련 할 수 있을 것 입니다.</p>
        </div>
        <div class="right_container">
            <img src="<?php echo G5_IMG_URL ?>/intro2_ceo.png">
            <p>아이디어허브(주) 대표 임경수</p>
        </div>
    </article>
</section>
<section id="our_mission">
    <article>
        <h3>Our Mission</h3>
        <div class="our_txt">
            <h2>아이디어허브는 2016년 설립된<br><span>특허수익화전문서비스</span> 기업입니다.</h2>
            <ol>
                <li>
                    <span>1</span>
                    <p>로얄티가 발생하고 있는 특허 수익권에 투자하여 안정된 수익을 얻을 수 있습니다.</p>
                </li>
                <li>
                    <span>2</span>
                    <p>개인/학교/연구소/기업 등 고객이 보유하고 있는 미활용 우수 특허를 <br class="pc">가치 있는 자산으로 수익화해드립니다.</p>
                </li>
                <li>
                    <span>3</span>
                    <p>아이디어허브가 확보한 특허로 다른 기업의 특허 공격을 방어해드립니다.</p>
                </li>
            </ol>
        </div>
        <div class="our_300">
            <div>
                <img src="<?php echo G5_IMG_URL ?>/ico_our_mission_heart.png">
                <b><span>300</span>건</b>
                <p>특허 라이센스 계약 체결 300건 이상</p>
            </div>
            <div>
                <img src="<?php echo G5_IMG_URL ?>/ico_our_mission_heart.png">
                <b><span>3,000</span>건</b>
                <p>
                    사업개시 후 5년 만에 이동통신, 차세개 코덱, <br class="pc">
                    스트리밍, 반도체, 클라우드 등의 <br class="pc">
                    분야에서 약 3,000여건의 특허 확보
                </p>
            </div>
            <div>
                <img src="<?php echo G5_IMG_URL ?>/ico_our_mission_heart.png">
                <b><span>300</span>건</b>
                <p>방대한 특허 포트폴리오를 기반으로<br>
                향후 5년간 예상 수입 3,000억원</p>
            </div>
        </div>
    </article>
</section>
<section id="our_history">
    <article>
        <h3>Our History</h3>
        <ul>
            <li>
                <div class="year">2022</div>
                <div class="history_list">
                    <p>특허전문관리기업 국내 최초 첫 IPO 준비중<br>
                    (12호) 알티미디어 특허 인수<br>
                    (13호) 중기 WI-FI 특허 인수 </p>
                </div>
            </li>
            <li>
                <div class="year">2021</div>
                <div class="history_list">
                    <p>(8호) IDC 특허 인수<br>
                    (9호) 연세대 WC 특허 인수<br>
                    (10호)WIFI 특허 인수(11호) KAIST 스트리밍 특허 계약<br>
                    (11호) KAIST 스트리밍 특허 계약<br>
                    벤처창업진흥 유공 (중기부장관표창 수상)<br>
                    Series B 투자 유치 (1200억 Value)</p>
                </div>
            </li>
            <li>
                <div class="year">2020</div>
                <div class="history_list">
                    <p>(5호) SK Planet DASH 특허 인수<br>
                    (6호)Pantech 특허 인수<br>
                    (7호)Viol(비올) Medical Device 특허 계약<br>
                    특허우산서비스 1호 계약</p>
                </div>
            </li>
            <li>
                <div class="year">2019</div>
                <div class="history_list">
                    <p>(3호) Cloud 특허 인수<br>
                    (4호) 3D-Package 특허 인수<br>
                    Series A 투자 유치 (500억 Value)</p>
                </div>
            </li>
            <li>
                <div class="year">2018</div>
                <div class="history_list">
                    <p>(2호) ETRI 스트리밍 특허 인수<br>
                    벤처기업 인증(기술보증 기금)<br>
                    부설연구소 설립인가 (KOITA)</p>
                </div>
            </li>
            <li>
                <div class="year">2017</div>
                <div class="history_list">
                    <p>2016년 11월 아이디어허브 설립<br>
                    수익화 서비스 1호 IoT 특허 인수</p>
                </div>
            </li>
        </ul>
    </article>
</section>
<section id="idb_ci">
    <article>
        <h3>CI 소개</h3>
        <p>모던하고 미래지향적인 심볼은 허브를 연상시키는 이니셜 i와 H의 합성으로 기업의 전문성과 체계적인 비즈니스를 표현합니다.</p>
        <div class="ci_wrap">
            <img src="<?php echo G5_IMG_URL ?>/ci_logo_1.png">
            <img src="<?php echo G5_IMG_URL ?>/ci_logo_2.png">
            <img src="<?php echo G5_IMG_URL ?>/ci_logo_3.png">
        </div>
    </article>
</section>
<section id="way_to_idb">
    <article>
        <div>
            <h3>찾아오시는 길</h3>
            <address>
            서울시 서초구 강남대로 251, 10층 (06735)<br>(3호선, 신분당선 양재역 2번 출구 도보 1분 거리)
            </address>
            <div class="contact">
                <p class="mail">070-7784-1643</p>
                <p class="tel">ideahub@ideahub.co.kr</p>
            </div>
        </div>
        <div id="map"></div>
    </article>
</section>

<script defer src="<?php echo G5_JS_URL ?>/index_map.js"></script>
<script defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyALgx-VXp6qO4fZ2Z5WCCLy1HtkMibDVOE&callback=initMap"></script>